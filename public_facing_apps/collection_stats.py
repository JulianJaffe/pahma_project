...
from connector import collectiondb
...
def getTopStats(dbsource, config):
    ...
    tableresults = collectiondb.latestcollectionstats(dbsource, "objByObjType", config)

    totalMusNo = totalObjNo = totalPieceNo = 0
    for i in xrange(len(tableresults)):
        totalMusNo += tableresults[i][1]
        totalObjNo += tableresults[i][5]
        totalPieceNo += tableresults[i][9]
...
def makeTable(code, longcode, dbsource, config):
    ...
    if code == 'total':
        return howToUse(code)

    tableresults = collectiondb.latestcollectionstats(dbsource, statgroup[code], config)
    res = ""
    ...
    if code == "efc":
        ...
        totalmusno, totalobjno, totalpieceno = 0, 0, 0
        for i in xrange(len(tableresults)):
            if tableresults[i][0] == "None":
                continue
            totalmusno += tableresults[i][1]
            totalobjno += tableresults[i][5]
            totalpieceno += tableresults[i][9]
    ...
    for i in xrange(len(tableresults)):
        ...
        label = makeprettylabel(labelverbatim)
        totalMusNoCount = tableresults[i][1]
        percentOfMuseumNos = round(tableresults[i][2], 3)
        totalObjectCount = tableresults[i][3]
        percentOfTotalObjects = round(tableresults[i][4], 3)
        trueObjectCount = tableresults[i][5]
        percentOfTrueObjects = round(tableresults[i][6], 3)
        totalPieceCount = tableresults[i][7]
        percentOfTotalPieces = round(tableresults[i][8], 3)
        truePieceCount = tableresults[i][9]
        percentOfTruePieces = round(tableresults[i][10], 3)
        dateOfCounts = tableresults[i][11]
...
def makeChart(code, dbsource, config):
    categoryDict = {'total': 'totalCounts', 'cont': 'objByCntntType', 'obj': 'objByObjType',
                    ...
                    'iot': 'objByImgObjType'}
    res = ""
    res += '     <div class="charts" id="%schart">\n' % code
    if code == 'total':
        res += writeIntro(dbsource, config)
        res += '   </div>\n'
        return res
        ...
        res += '    <div id="%schart_met"></div><div id="%schart_cat"></div>%s<div id ="%struechart"></div></div>\n' % (code, code, extra, code)
        chart = buildCombo(code, dbsource, config, categoryDict[code])
    else:
        res += '    <div id="%schart_met"></div><div id ="%struechart"></div></div>\n' % (code, code)
...
def buildCombo(code, dbsource, config, longcode):
    ...
    if code == 'cont':
        ...
        for metric in ['totalMusNoCount', 'trueObjectCount', 'truePieceCount']:
            data = collectiondb.lateststatsforstatgroupbycounttype(dbsource, longcode, metric, config)
...
def writeIntro(dbsource, config):
    ...
    return '''<b><center>How many objects are in the Hearst Museum's collections?</center></b>
<br><br>
...
def makeTime(code, config):
    ...
    if code == 'total':
        totFlag = 1
    res += '''      <div class="time" id="%stime"><div id="%s%s_met"></div>\n''' % (code, code, categoryDict[code])
    if not totFlag:
        res += '''<div id="%s%s_cat"></div>\n''' % (code, categoryDict[code])
    res += '<div id="%slinetime"></div><div id="%scontrol"></div></div>\n' % (code, code)
    chart = '''<script type="text/javascript">
     google.load('visualization', '1', {'packages':['corechart', 'controls']});
     function drawVisualization() {'''
...
def prepTime(code, category, config):
    ...
    for group in ['musNumbers', 'objects', 'pieces']:
        total = {}
        tableresults = collectiondb.getStatSeries(group, category, config)
